...
def fetch_last_processed_post_number():
    ...
    print("Last processed post number",metadata["value"])
...
def fetch_existing_sentences(last_post_number):
    """Fetch all existing sentences."""
    query = {
        "post_number": {"$lte":last_post_number},
        "embedded flag": True,
        "process flag": True
    }
    return list(posts_collection.find(query))
...
def batch_job():
    ...
    existing_sentences = fetch_existing_sentences(last_processed_post_number)
    ...
    for new_sentence in new_sentences:
        for existing_sentence in existing_sentences:
            ...
            if new_sentence["user_id"] != existing_sentence["user_id"]:
                ...
                if str(new_sentence["_id"]) < str(existing_sentence["_id"]):
